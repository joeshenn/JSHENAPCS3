...
public class FormulaCell extends RealCell {
	private Spreadsheet accessCell;
	...
	public double getDoubleValue(){
		...
		if(longInput[1].toUpperCase().equals("SUM") || longInput[1].toUpperCase().equals("AVG")) {
		...
		//	System.out.println(numTerms);
			...
			if(longInput[1].toUpperCase().equals("SUM")) {
				return sum;
			}
			else  {  //AVG
				return sum / numTerms;
		...
		for(int j=1;j<longInput.length;j++) {
			if(longInput[j].toUpperCase().charAt(0) >= 65 && longInput[j].toUpperCase().charAt(0) <= 76) {
				SpreadsheetLocation refCell = new SpreadsheetLocation(longInput[j].toString().toUpperCase());
